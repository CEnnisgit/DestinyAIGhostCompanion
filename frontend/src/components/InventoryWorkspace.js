...
const ACTION_LABELS = {
  ...
  pull_from_postmaster: 'Pull from Postmaster',
  move_to_character: 'Move to Character',
};
...
function InventoryWorkspace({
  ...
  const filteredSections = useMemo(() => {
    ...
    Object.entries(sections).forEach(([key, items]) => {
      next[key] = (items || []).filter((item) => {
        const text = [
          ...
          item.item_type_name,
          item.tier,
          characters.find((character) => character.character_id === item.character_id)?.class_name,
        ]
          .filter(Boolean)
          .join(' ')
          .toLowerCase();
    ...
    return next;
  }, [characters, normalizedQuery, sections]);
    ...
    <section className="inventory-panel" aria-label="Inventory management">
      <div className="inventory-panel__header">
        <div>
          ...
          {lastUpdated && <span className="inventory-panel__timestamp">Updated {lastUpdated}</span>}
        </div>
      ...
      <div className="inventory-panel__toolbar">
        <input
          type="search"
          className="inventory-panel__search"
          value={query}
          onChange={(event) => setQuery(event.target.value)}
          placeholder="Search gear, tier, or class..."
        ...
        {Object.entries(SECTION_LABELS).map(([key, label]) => (
          <section key={key} className="inventory-section">
            <header className="inventory-section__header">
              <h3>{label}</h3>
              <span>{(filteredSections[key] || []).length}</span>
            </header>
            <div className="inventory-section__list">
              {(filteredSections[key] || []).length === 0 && (
                <div className="inventory-card inventory-card--empty">
                  {normalizedQuery ? 'No matching items in this section.' : `No ${label.toLowerCase()} items available.`}
                </div>
              )}
                ...
                return (
                  <article key={item.instance_id} className="inventory-card">
                    <div className="inventory-card__meta">
                      ...
                      <div>
                        <h4>{item.name}</h4>
                        <p>{[item.item_type_name, item.tier, item.class_name].filter(Boolean).join(' - ')}</p>
                        <p>
