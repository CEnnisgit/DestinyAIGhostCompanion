...
const MessageInput = ({ onSend, onVoiceInput, disabled, speakEnabled, onSpeakChange, selectedVoice, onVoiceChange, availableVoices, micDeviceId, micDetectionError, serverMicAvailable, serverMicDefaultIndex, ttsActive, onVoiceInterrupt, voiceStatusLabel, interactionBlocked }) => {
  ...
  }, [onSend, onVoiceInterrupt, stopRecognitionSession]);
  ...
  const startMediaRecorder = useCallback(async () => {
    ...
    setListening(true);
  }, [cleanupMediaStream, micDeviceId, onVoiceInterrupt]);
  ...
  const startServerSideRecording = useCallback(
    ...
    [onSend, onVoiceInterrupt, serverMicAvailable, serverMicDefaultIndex]
  );
  ...
  const statusMessage = listening
    ...
    : (transcribing ? 'Transcribing\u2026' : (voiceStatusLabel || speechStatus));
  ...
  useEffect(() => {
    ...
    if (disabled || transcribing || listening || interactionBlocked) {
      return;
    }
    if (ttsActive) {
      return;
  ...
  }, [speakEnabled, disabled, transcribing, listening, micDetectionError, value, startRecording, stopRecording, ttsActive, interactionBlocked]);
